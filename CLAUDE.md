...
The RL stack has an automated harness (no editor needed): `cd Python; venv\Scripts\python.exe smoke_test.py` runs the full training loop against a mock UE bridge (`Python/tests/mock_ue_server.py`) in ~3 s, and `Python/tests/` holds the pytest suite (boss_env, rl_algo, wrappers, replay schema). `venv\Scripts\python.exe eval_archetypes.py` scores every exported brain in `SourceArt/Models/` against a seeded scripted duel offline (~10-20 s per brain, no UE, no onnxruntime — a built-in numpy interpreter runs the exported MLP): reward, W/L/D, action distribution, mask-violation gate (must be 0), entropy, rusher-vs-turtle divergence verdict. This is the promotion bar for new checkpoints (turtle was promoted on a 40-0 result; `--all` includes the untrained baseline). In-game feel/combat testing remains manual: UE editor with the level open + `infer.py`/`train.py`, verified in-game and via TensorBoard.
...
- **`UGameFeelSettings`** (UDeveloperSettings → Project Settings → Game → Game Feel; persisted in `Config/DefaultGame.ini`) owns all cross-cutting feel numbers: camera (FOV/arm/lag/lock-on), telegraph colors (red unblockable `#FF2A1A` / yellow parryable `#FFC400`), boss-bar chip timing, plus `bEnableBossStatusHUD` / `bEnableCombatCamera` / `bApplyCameraDefaults` toggles. Also owns the NNE brain wiring: `bEnableNNEBoss`, `NNEBossModelData`, and the `NNEArchetypeBank` persona rows (see Ship Path). Tune here, never rebuild.
...
- **`UNNEBossPolicyComponent`** (auto-injected on the boss via `UGameFeelSubsystem` when `UGameFeelSettings::bEnableNNEBoss`): in-engine ONNX inference (NNERuntimeORTCpu) at 15 Hz — mirrors `StateObservationComponent`'s exact 17-dim obs, applies `GetLegalActionMask()` to the logits, argmax → `ExecuteAction`. **Arbitration: `-rlbridge` or a live TCP client always outranks NNE** (training never fights the shipped brain; `run_training.ps1` passes `-rlbridge -NoTelemetry`). Model wiring: `GameFeelSettings::NNEBossModelData` is the default brain (currently `NNM_BossTurtle`, the 110k-step checkpoint promoted on eval_archetypes.py evidence); new brains go `Python/export_onnx.py` → `Tools/import_onnx_model.py` (headless, env-parameterized via `NNE_ONNX_SRC`/`NNE_ASSET_NAME`) → eval gate → wiring. Imported brains live in `/Game/Arena/Models` (force-cooked via `DirectoriesToAlwaysCook`; ONNX sources in `SourceArt/Models/`). Archetype bank: settings-driven `+NNEArchetypeBank=(Persona=...,ModelData=...,Centroid=(8 floats))` rows in `DefaultGame.ini` auto-populate the component's `TMap<FName, UNNEModelData>` bank (a BP-authored bank/ArchetypeProfiles asset outranks them); selection is MEAN-CENTERED cosine (both vectors centered at the 0.5-neutral profile — raw cosine saturates on all-positive vectors) against the player's stored `PlayerMemoryComponent` profile, restricted to personas with a bank model. Centroids are MEASURED (`Python/measure_centroids.py` — all-steps mean profile from `replays/<persona>/`; keep its math in sync with `SelectFromEntries`). **Memory lifecycle is owned by `UGameFeelSubsystem`** (without it the bank is dead code): it calls `LoadMemory` (Firebase UID, else `"guest"`) on the boss's `PlayerMemoryComponent` BEFORE injecting the NNE component (whose BeginPlay resolves the archetype match), and `RecordEncounterEnd`+`SaveMemory` at every round end (hero `OnHealthDepleted` / boss `OnBossDied`, 2 s debounce); recording is skipped under `-rlbridge`/live TCP client so bot rounds never pollute the human dossier. Self-test: `boss.NNESelfTest` console command (pre-world fallback reads `GameFeelSettings.NNEBossModelData`, so it exercises the shipped brain).
...
| Tests | `smoke_test.py` (end-to-end vs mock bridge), `eval_archetypes.py` (offline brain eval / promotion gate), `tests/` (mock_ue_server.py + pytest suite) |
| M5 export | `export_onnx.py`, `make_test_onnx.py`, `measure_centroids.py` |
...
- `Website/` — the player dashboard ("Subject Dossier": Firebase auth, profile radar, emotion timeline, fight log, taunt panel, community "One Boss" World page (`/world`, global stats from Firestore `meta/global`), download page). React + Vite; builds clean; demo mode works without Firebase. Setup + the **canonical Firestore schema** (the contract the game's uploader must write) live in `Website/README.md`.
...
- `Python/export_onnx.py` — SB3 checkpoint → ONNX for in-engine NNE inference, with agreement verification. `Python/make_test_onnx.py` generates the untrained placeholder (NNM_BossUntrained) for pipeline validation.
- `Python/eval_archetypes.py` — offline brain eval vs a seeded scripted duel (see Testing) — the promotion gate for exported checkpoints.
- `Python/measure_centroids.py` — per-persona 8-dim behavior centroids from `replays/<persona>/` (all-steps mean profile + centered-cosine separation matrix); prints ready-to-paste `+NNEArchetypeBank` ini rows. Re-run after every overnight run batch.
- `Tools/import_onnx_model.py` — headless ONNX → `UNNEModelData` importer (`-ExecutePythonScript`); env-parameterized via `NNE_ONNX_SRC`/`NNE_ASSET_NAME` for the per-persona archetype rotation; force-save + mtime verify.
- `Tools/fix_montage_slots.py` — repoints every combat montage's slot track to a slot that exists in the AnimBP (fixes invisible attacks from retargeted montages inheriting source-pack slot names); editor Cmd `py`.
- `Tools/disable_hit_debug_spheres.py` — unticks `bDrawDebugTrace` on every ANS_DealDamage notify instance (removes the M0 debug spheres); editor Cmd `py`.
- `Tools/integrate_golem_minion.py` — swaps BP_Minion visuals from mannequin placeholder to the Stone Golem Fab pack; degrades loudly if the golem retarget batch is missing; re-runnable.
- `Tools/equip_weapons_and_trails.py` — socket-attached weapon meshes on boss/hero + Niagara slash trails on attack montages.
